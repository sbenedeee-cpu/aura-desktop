...
Migrations are numbered, append-only, and applied inside a SQLite transaction. If a migration fails, Aura does not reset the database; it reports that the prior records were left unchanged so the user can restart an updated build or retain a copy of the database before further recovery work. The local data-encryption boundary is in place under [ADR-004](docs/decisions/ADR-004-encryption-strategy.md): a random 32-byte data-encryption key is wrapped with Windows DPAPI (only the wrapped blob `aura.keywrap` is persisted, and the raw key never touches disk) and secrets flow through versioned ChaCha20Poly1305 envelopes that fail authentication when tampered. The key-wrapping path was implemented against the Windows bindings; a real-Windows build verification remains the final confirmation before release.
