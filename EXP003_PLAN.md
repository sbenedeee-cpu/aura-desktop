...
## CI merge-gates task state (2026-08-14)
User asked to enable CI quality checks as required merge gates on repo sbenedeee-cpu/aura-desktop.
Plan: 1) CI workflow file `.github/workflows/ci.yml` ALREADY EXISTS (Aura Quality Gate, jobs `renderer` and `native` on ubuntu-latest; runs on PRs to main and pushes to main; concurrency group; rust-cache; cargo-audit on Cargo.lock; pnpm quality:renderer + security:js). I edited it: renamed jobs to `quality-renderer`, `quality-native` (stable status check names) and ADDED a new job `tauri-build-windows` (runs-on: windows-latest: checkout, pnpm, node 22, dtolnay/rust-toolchain@stable, pnpm install --frozen-lockfile, `pnpm tauri build -- --no-bundle`, then pwsh smoke check of src-tauri/target/release/aura-desktop.exe, upload-artifact@v4 name aura-desktop-windows-x64 retention 7d).
NOT yet: commit/push, verify workflow passes on main, then configure required status checks via gh api: gh api repos/sbenedeee-cpu/aura-desktop/branches/main/protection with required_status_checks (contexts: quality-renderer, quality-native, tauri-build-windows), enforce_admins true?, also set merge options (require_pull_request_reviews maybe not — user only asked quality checks), allow merge commit/squash. Check current protection first: gh api repos/.../branches/main/protection.
Quality scripts: pnpm quality:renderer (format:check, lint, typecheck, test, build), security:js, quality:native (rust fmt/clippy/test), cargo audit.
Repo default branch: main. gh CLI logged in.
